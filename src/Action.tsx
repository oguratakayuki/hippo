...
export default function Action(props: ActionProps) {
  ...
  const actionTypeChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const actionType1Elem = document.getElementById(
      "actionType1"
    ) as HTMLSelectElement;
    const actionType1: string = String(
      actionType1Elem.options[actionType1Elem.selectedIndex].value
    );
  ...
  const breedAction = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const actionType1: string =
      actionType1Elem.options[actionType1Elem.selectedIndex].value;
    const actionType2Elem = document.getElementById(
      "actionType2"
    ) as HTMLSelectElement;
    const actionType2: string =
      actionType2Elem.options[actionType2Elem.selectedIndex].value;
    const actionValueElem = document.getElementById(
      "actionValue"
    ) as HTMLInputElement;
  ...
  return (
    <form onSubmit={breedAction}>
      <div className="action-form">
        ...
        <div className="mb-3">
          <label htmlFor="option-input2" className="form-label">
            action option
          </label>
